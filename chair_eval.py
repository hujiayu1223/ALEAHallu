...
from minigpt4.Halle_Editor.prompt_tuning import add_diffusion_noise
...
def draw(
    ...
    attn_square,
    save_path,
    image_token_range=(34, 610),
    generated_token_start=637,
    patch_grid=(24, 24),
    colormap=cm.plasma,
    alpha=0.5
):
    ...
    image_token_range = range(image_start, image_end)
    # generated_token_range = range(generated_token_start, attn.shape[0])
    generated_token_range = range(generated_token_start-1, generated_token_start)

    # 取出生成 token 的 attention 向量，对 image token 部分
    attn_to_image = attn[generated_token_range, :][:, image_token_range]

    # 对生成 token 求平均
    mean_attention = attn_to_image.mean(dim=0)

    # print(mean_attention.min()) # 0.0002 0.0003
    # print(mean_attention.max()) # 0.0309 0.0908
    # exit()

    mean_attention = mean_attention - 0.0002
    if mean_attention.max() > 0:
        mean_attention = mean_attention / 0.0908

    # mean_attention = mean_attention.pow(0.7)


    # print("attn shape:", attn.shape)  # expect [772, 772]
    # print("attn_to_image shape:", attn_to_image.shape)  # expect [162, 576]
    # print("mean_attention shape:", mean_attention.shape)  # expect [576]
    # print(mean_attention)

    # Reshape to patch grid
    attn_2d = mean_attention.reshape(patch_grid).cpu().detach().numpy()

    # Resize attention map to original image size
    W, H = raw_image.size
    attn_colored = np.uint8(colormap(attn_2d)[:, :, :3] * 255)  # RGB heatmap
    attn_img = Image.fromarray(attn_colored).resize((W, H), resample=Image.BILINEAR).convert("RGBA")

    # Original image as RGBA
    raw_rgba = raw_image.convert("RGBA")

    # Overlay
    blended = Image.blend(raw_rgba, attn_img, alpha=alpha)
...
def _draw(
    ...
    pre_attn = pre_attn_square[0]
    pos_attn = pos_attn_square[0]
    image_start, image_end = image_token_range
    image_token_range = range(image_start, image_end)
    # generated_token_range = range(generated_token_start, attn.shape[0])
    generated_token_range = range(generated_token_start-1, generated_token_start)
    ...
    # 取出生成 token 的 attention 向量，对 image token 部分
    pre_attn_to_image = pre_attn[generated_token_range, :][:, image_token_range]
    pos_attn_to_image = pos_attn[generated_token_range, :][:, image_token_range]

    # 对生成 token 求平均
    pre_mean_attention = pre_attn_to_image.mean(dim=0)
    pos_mean_attention = pos_attn_to_image.mean(dim=0)

    _min_ = min(pre_mean_attention.min(),pos_mean_attention.min())
    _max_ = max(pre_mean_attention.max(),pos_mean_attention.max())

    if is_pre:
        mean_attention = pre_mean_attention
    else:
        mean_attention = pos_mean_attention

    mean_attention = mean_attention - _min_
    if mean_attention.max() > 0:
        mean_attention = mean_attention / _max_
...
# pre_data = torch.load("/home/hjy/work/opera/attn_outputs/pre_attn.pt")
# pos_data = torch.load("/home/hjy/work/opera/attn_outputs/pos_attn.pt")

# pre_sum = 0
# pos_sum = 0
# front_prompt_token_range = range(0,34)
# image_token_range = range(34, 610)
# latter_prompt_token_range = range(610,636)
# last_input_token_range = range(636, 637)
# for idx in range(len(pre_data)):
#     front_prompt_sum = pre_data[idx]["attn_square"][0][last_input_token_range, :][:, front_prompt_token_range].sum(dim=1)
#     image_sum = pre_data[idx]["attn_square"][0][last_input_token_range, :][:, image_token_range].sum(dim=1)
#     latter_sum = pre_data[idx]["attn_square"][0][last_input_token_range, :][:, latter_prompt_token_range].sum(dim=1)
#     pre_sum += image_sum / (front_prompt_sum + image_sum + latter_sum)

#     front_prompt_sum = pos_data[idx]["attn_square"][0][last_input_token_range, :][:, front_prompt_token_range].sum(dim=1)
#     image_sum = pos_data[idx]["attn_square"][0][last_input_token_range, :][:, image_token_range].sum(dim=1)
#     latter_sum = pos_data[idx]["attn_square"][0][last_input_token_range, :][:, latter_prompt_token_range].sum(dim=1)
#     pos_sum += image_sum / (front_prompt_sum + image_sum + latter_sum)
#     # pre_sum += pre_data[idx]["attn_square"][0][last_input_token_range, :][:, image_token_range].sum(dim=1)
#     # pos_sum += pos_data[idx]["attn_square"][0][last_input_token_range, :][:, image_token_range].sum(dim=1)
# print("pre_average:", pre_sum/500)
# print("pos_average:", pos_sum/500)
# exit()
...
for img_id in tqdm(range(4000,4500)): # range(4000,4500) id_list[:1500]
    ...
    # qu_norm = "Please describe this blurry image in detail, no need to mention the blurriness of the image."
    ...
    image = vis_processors["eval"](raw_image).unsqueeze(0)
    # image_cd = add_diffusion_noise(image, 500).to(device)
    # soft_prompt = torch.load('/home/hjy/work/opera/edited_model/soft_prompt_epoch_4_token_5.pt')
    img[img_id] = image
    ...
    with open(os.path.join(base_dir, 'llava_next_prompt_learning_500.jsonl'), "a") as f:
        json.dump(img_save, f)
        f.write('\n')
...
save_path = f'{args.results_save_dir}/llava_next_model'
