...
class GenerationMixin:
    ...
    def generate(
        self,
        inputs: Optional[torch.Tensor] = None,
        response_start: Optional[List[int]] = None,
        ...
        prompt_learn: Optional[bool] = False,
        ...
        **kwargs,
        ...
        if images_cd is not None:
            model_kwargs["images_cd"] = images_cd
...
# 这个elif结构在opera对比实验中需要用到
        ...
        #     )
            ...
            # 13. run beam search   #直接返回这个
            if not generate:  # 当不需要生成的时候直接返回logits值,且不需要对inputs_ids进行expand
                model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
                outputs = self(
                    response_start=response_start,
                    prompt_learn=prompt_learn,
                    prompt_vec=prompt_vec,
                    **model_inputs
                )
                return outputs.logits,None
            else:
                # 12. interleave input_ids with `num_beams` additional sequences per batch 在这里对input_ids乘上num_beams的数量
                input_ids, model_kwargs = self._expand_inputs_for_generation(
                    input_ids=input_ids,
                    expand_size=generation_config.num_beams,
                    is_encoder_decoder=self.config.is_encoder_decoder,
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # Store scores, attentions and hidden_states when required
            # attn_square += (outputs.attentions,) # 记录输出的attn用于计算在image token上的值
            if return_dict_in_generate:
                if output_scores:
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        pad_token_id = pad_token_id if pad_token_id is not None else self.generation_config.pad_token_id #填充符号
        eos_token_id = eos_token_id if eos_token_id is not None else self.generation_config.eos_token_id #结束符号
        ...
        # output_hidden_states = True  # 强制为True
        # return_dict_in_generate = True # 强制为True
        ...
        # print(self.model) self.model是一个多模态的LlavaLlamaModel
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs) # 输入多条input_ids
            outputs = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
            ...
            # outputs.attentions[-1].size(): [5,32,623,623],这里是指attention的最后一层
            # outputs.attentions包含了32层注意力层的输出
            if not "past_key_values" in model_kwargs.keys():
                attn_previous = outputs.attentions[
                    -1].clone()  # [batch_size * num_beams, num_head, q, kv] 初始化beam_search的路径，维度为1是指多头注意力
                output_begin_attn = outputs.attentions[0].size(-1)
                # outputs.logits.size() [5,623,32000] [batch_size,q,vocab_size]
                logit_previous = outputs.logits.clone()
                output_logit_begin = outputs.logits.clone()
            else:
                assert beam_idx is not None and attn_previous is not None and logit_previous is not None
                attn_previous = torch.cat([attn_previous, torch.zeros_like(attn_previous).sum(-1, keepdim=True)], -1)
                attn_previous = torch.cat(
                    [attn_previous[beam_idx], outputs.attentions[-1].clone().max(1, keepdim=True).values.data],
                    -2)  # [batch_size * num_beams, num_head, q, kv]
                logit_previous = torch.cat(tensors=[logit_previous,outputs.logits.clone()],dim=-2)
                # 上一个beam_index对应的attn值与目前分数最好的进行拼接。

            attn_previous = attn_previous.max(1, keepdim=True).values.data  # [batch_size * num_beams, 1, q, kv]
            if synced_gpus and this_peer_finished:
                cur_len = cur_len + 1
                continue  # don't waste resources running the code we don't need
            # print("attn:",outputs.attentions[-1].shape)
            next_token_logits = outputs.logits[:, -1, :]
            # print("logit:",next_token_logits[0])
            # hack: adjust tokens for Marian. For Marian we have to make sure that the `pad_token_id`
            # cannot be generated both before and after the `nn.functional.log_softmax` operation.
            next_token_logits = self.adjust_logits_during_generation(next_token_logits, cur_len=cur_len)
            next_token_scores = nn.functional.log_softmax(
                next_token_logits, dim=-1
            )  # (batch_size * num_beams, vocab_size) softmax之后就变成了分数，但只是logits结果，不是注意力分数
            next_token_scores_processed = logits_processor(input_ids, next_token_scores)
            next_token_scores = next_token_scores_processed + beam_scores[:, None].expand_as(next_token_scores) #因为已经log过所以算概率只要相加即可
            # Store scores, attentions and hidden_states when required
            if return_dict_in_generate:
                if output_scores:
            ...
            next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size)  #把所有候选beam序列相连
            # Sample 2 next tokens for each beam (so we have some spare tokens and match output of beam search) 每个beam选择两个最好的tokens
            next_token_scores, next_tokens = torch.topk(
                next_token_scores, 2 * num_beams, dim=1, largest=True, sorted=True
            )
            next_indices = torch.div(next_tokens, vocab_size, rounding_mode="floor") #按voc的大小进行分段,indices表示第几个beam
            ...
            # print("beam_next_tokens:",beam_outputs["next_beam_tokens"])
            input_ids = torch.cat([input_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
            # print("input_ids:",input_ids[beam_idx])
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
            ...
            if beam_scorer.is_done or stopping_criteria(input_ids, scores):
                if not synced_gpus:
                    ...
                    this_peer_finished = True
        # print("beam_score:",beam_scores) #计算最后的得分来判断用的是哪个beam
        attn_idx = torch.argmax(beam_scores)
        halle_hidden = ()
        # for hidden_state in outputs.hidden_states:
        #     halle_hidden += (hidden_state[attn_idx,:,:],)
        ...
        if return_dict_in_generate:
            ...
            attn_square = attn_previous[attn_idx,:,:,:]
            # attn_square = attn_square.detach().cpu().numpy()[0]
            # return output_logit_begin,sequence_outputs["sequences"]
            return output_logit_begin,sequence_outputs["sequences"]
    ...
    def opera_beam_search_rev(
        ...
        **model_kwargs,
        ...
        batch_beam_size, cur_len = input_ids.shape # input_ids=[序列个数,序列长度]
        ...
        while True:
            ...
            # Load the previous self-attention weights
            if not "past_key_values" in model_kwargs.keys():
                attn_previous = outputs.attentions[-1].clone() # [batch_size * num_beams, num_head, q, kv] 初始化beam_search的路径
            else:
                assert beam_idx is not None and attn_previous is not None
                # print("curr_beam_attn_size,next_attn_size",attn_previous[beam_idx].size(),outputs.attentions[-1].clone().max(1, keepdim=True).values.data.size())
