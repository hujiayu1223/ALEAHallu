...
def multiclass_log_probs(config, pred, targ, shift=False, eps=torch.finfo(torch.float32).eps, exact_match=False,
                         **kwargs):
...
def log_probs(config, pred, targ, shift=False, exact_match=False, **kwargs):
    NULL_TOKEN = 0
    pred = pred.to(torch.float32)
    pred = pred.clone()
    targ = targ.clone()
    pred = pred[:, -targ.size(1):] #默认pred的长度要比target长，可以直接取target长度，从后往前取
    ###############
    mask = targ != -100
    ...
    unmasked_log_probs = pred.log_softmax(-1).gather(-1, targ.unsqueeze(-1)).squeeze(-1)
    # 这里是取出pred中对应target的相应log_softmax结果
    pred_ids = pred.argmax(-1).masked_fill(~mask, NULL_TOKEN)
    correct = pred_ids == targ
    correct = correct & mask
    num_non_padding = mask.sum().float().item() # 非填充的token个数
    acc = correct.sum() / num_non_padding
    n_tokens = mask.float().sum()
    print(n_tokens,targ.size(1))
    log_prob = (unmasked_log_probs * mask.float()).sum() / n_tokens
    return -log_prob
