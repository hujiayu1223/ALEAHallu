...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def generate(
        ...
        if image.dim() == 5:
            ...
            input_llm_list = []
            for image in [image,images_cd]:
                if image is None:
                    input_llm_list.append(image)
                    continue
                with self.maybe_autocast():
                    image_embeds = self.ln_vision(self.visual_encoder(image))
                image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
                ...
                input_llm_list.append(inputs_llm)
        ...
        llm_tokens = self.llm_tokenizer(
            ...
            padding="longest",
            return_tensors="pt"
        ).to(input_llm_list[0].device)
        with self.maybe_autocast():
            inputs_llm,inputs_llm_cd = input_llm_list
            inputs_embeds_ord = self.llm_model.get_input_embeddings()(llm_tokens.input_ids)
            inputs_embeds = torch.cat([inputs_llm, inputs_embeds_ord], dim=1)
            if images_cd is not None:
                inputs_embeds_cd = torch.cat([inputs_llm_cd, inputs_embeds_ord], dim=1)
            # print("inputs_llm:",inputs_llm.size(),"inputs_embeds:",inputs_embeds.size())
            images_cd = inputs_embeds_cd if images_cd is not None else images_cd
            attention_mask = torch.cat([atts_llm, llm_tokens.attention_mask], dim=1)
            ...
            logits,outputs = self.llm_model.generate(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
