...
class MiniGPT4(Blip2Base):
    ...
    def generate(
        ...
        image = samples["image"]
        img_embeds, atts_img = self.encode_img(image)
        # img_embeds_cd, _ = self.encode_img(images_cd)
        if self.prompt_list:
            instruction = random.choice(self.prompt_list)
        else:
        ...
        # inputs_embeds_cd, _,_ = self.prompt_wrap(img_embeds_cd, atts_img, instruction)
        batch_size = img_embeds.shape[0]
        bos = torch.ones([batch_size, 1],
                         dtype=torch.int64,
        ...
        with self.maybe_autocast():
            inputs_embeds = torch.cat([bos_embeds, inputs_embeds], dim=1)
            # inputs_embeds_cd = torch.cat([bos_embeds,inputs_embeds_cd],dim=1)
            attention_mask = torch.cat([atts_bos, attention_mask], dim=1)
            ...
            logits,outputs = self.llama_model.generate(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
