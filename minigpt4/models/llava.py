...
class LLaVa(BaseModel):
    ...
    def __init__(
        ...
        self.llama_model = LlavaLlamaForCausalLM.from_pretrained(
            merged_ckpt, low_cpu_mem_usage=True, output_hidden_states = True, **kwargs)
        # self.prepare_input = self.llama_model.prepare_inputs_labels_for_multimodal()
        # nullu :/home/hjy/work/Nullu/output/edited_model/LLaVA-7B-top4-16-32-test
    ...
    def forward(self, samples):
        ...
        attention_mask = input_ids.ne(self.llama_tokenizer.pad_token_id).long()
        past_key_values,labels = None,None
        # print(input_ids.shape, attention_mask.shape, image.shape)
        input_ids, attention_mask, past_key_values, inputs_embeds, labels = self.llama_model.prepare_inputs_labels_for_multimodal(
            input_ids, attention_mask, past_key_values, labels, image)
    ...
    def generate(
        ...
        instruction = samples["prompt"] if "prompt" in samples else None
        bs = image.size(0) #由于在DINM中一张图片是对应两个prompt的所以bs不能这么算
        if isinstance(instruction, str):
            instruction = [instruction] * bs
        else:
            # assert len(instruction) == bs*2, "The number of prompts must be equal to the batch size."
           assert len(instruction) == bs
        instruction = [self.system_message + p  for p in instruction]
        # instruction = self.system_message + instruction

        # print(instruction)
        # for ins in range(len(instruction)):
        #     instruction[ins] += generation[ins]
        chunks_before, chunks_after = [], []
        for p in instruction:
            chunk_before, chunk_after = p.split('<ImageHere>')
            chunks_before.append(chunk_before)
            chunks_after.append(chunk_after)
        # x = [text.split(' ') for text in chunks_before]
        # count = 0
        # for input_words in x[0]:
        #     tokens_before = self.llama_tokenizer(
        #         input_words,
        #         return_tensors="pt",
        #         padding="longest",
        #         add_special_tokens=False
        #     ).to(image.device).input_ids
        #     print(count, input_words, tokens_before)
        #     count += len(tokens_before[0])
        #instruction和prompt会根据image的位置构造成tokens_before和token_after两个位置
        tokens_before = self.llama_tokenizer(
            chunks_before,
            return_tensors="pt",
            padding="longest",
        ...
        with self.maybe_autocast():
            ...
            if key_position is None:
                key_position = {
                    "image_start": tokens_before.shape[1]+1,  # pope: idx = 34 时是image_token , NUM_IMAGE_TOKENS = 576, 那么在image后面的第一个token位置是 34+576=610
                    "image_end": tokens_before.shape[1]+NUM_IMAGE_TOKENS, 
                    "response_start": input_ids.shape[1]+NUM_IMAGE_TOKENS-1, # qu_halle: 650
                }
            # print("image_start:%d",key_position["image_start"])
            # print("image_end:%d",key_position["image_end"])
            # print("response_start:%d",key_position["response_start"])
            # exit()
            # image_start = key_position["image_start"]
            # image_end = key_position["image_end"]
            #print("llama_model.generate:",self.llama_model.generate.__module__) 这里直接调用的transformers.generation.utils里的generate
            # print(image.shape)  [1,3,336,336] 3为RGB图片的通道数
            # output_ids,hidden_state = self.llama_model.generate(
            logits,output_ids = self.llama_model.generate(
                input_ids=input_ids,
                response_start = key_position["image_end"]+1,
                use_cache=True,
        ...
        if output_ids is not None:
            ...
            output_text = self.llama_tokenizer.batch_decode(output_ids[:, input_token_len:], skip_special_tokens=True) # output是包括了input在内的，所以要从input长度之后开始decode输出
            output_text = [text.split('###')[0].strip() for text in output_text]

    #     #####################################################
    #     #         将output_id与token和输出文本进行分别对应        #
    #     #                                                   #
    #     #####################################################

        # x = [text.split(' ') for text in output_text]
        # count = 0
        # for idx,input_words in enumerate(x[0]):
        #     tokens_before = self.llama_tokenizer(
        #         input_words,
        #         return_tensors="pt",
        #         padding="longest",
        ...
        #     ).to(image.device).input_ids
        #     # print(count, input_words, tokens_before)
        #     if input_words == "plate":
        #         hallu_idx = count
        #         hallu_len = len(tokens_before[0])
        #     print(count,input_words,tokens_before[0])
        #     count += len(tokens_before[0])
        # print(output_ids[:,input_token_len+hallu_idx])
        # exit()
        # hallu_sum = torch.sum(attn[0][input_token_len+hallu_idx,image_start:image_end],dim=1)
        # return attn,output_text,{"image_start":image_start,"image_end":image_end,"response_start":input_token_len,"hallu_idx":hallu_idx,"hallu_len":hallu_len}
        return logits,output_text
    #################################
